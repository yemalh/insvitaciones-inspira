// Edit this file to configure your event details
export const eventConfig = {
  eventName: 'Experiencia Inspira',
  eventDate: '16 de mayo, 2026',
  eventTime: 'Ver horario seleccionado',
  timeSlots: ['8:00 AM', '9:30 AM'],
  location: 'Álamos Roof, Querétaro',
  maxCapacity: 30,
  contactWhatsApp: '+524461073493',
  welcomeText:
    'Tu presencia hace posible este espacio de respiración, meditación y contemplación.',
  brand: {
    name: 'INSPIRA',
    tagline: 'Respiración · Meditación · Contemplación',
    slogan: 'Inhala. Conecta. Transforma.',
  },
}
